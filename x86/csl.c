...
static int find_ram_region_limit(const uint64_t where, uint64_t *const limit)
{
...
static int ram_region_check(const uint64_t address, const uint64_t size,
		const char *const cmd_name)
{
	uint64_t ram_region_limit;
